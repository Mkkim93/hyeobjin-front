...
export default {
    ...
    $('#summernote').summernote({
      ...
      callbacks: {
        ...
        onMediaDelete: ($target) => {
          ...
          const fileUrl = $target.attr('data-filename');
          console.log('fileUrl', fileUrl);
          this.deleteImageFromServer(fileUrl);
  ...
  methods: {
    ...
    async deleteImageFromServer(fileName) {
      try {
        const response = await this.$axios.delete(`/admin/boardfiles/delete?fileName=${fileName}`);
        console.log(response);
        console.log("게시글 수정 중 파일 삭제 완료");
      } catch (error) {
        console.log('게시글 파일 삭제 실패', error);
      }
    },
  },
}
